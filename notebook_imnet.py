...
from utilities import imagenet_utils as imgnet_utils
...
parser.add_argument('--dataset', type=str, choices=['imagenet'], help='Dataset to use.')
...
parser.add_argument('--max_iter_cs', type=int, default=100000, help='maximum number of batch iterations for cs criterion')
...
parser.add_argument('--scaling_attenuation', type=float, default=0.2, help='global attenuation factor for weights')
...
class Mask:
    ...
    def computeDiscrScores(self):
        ...
        niter = 0
        for X, Y in self.train_prune_loader:
            X = X.cuda()
            Y = Y.cuda()
    ...
    def _cmpInvCardinalityMat(self):
        ...
        niter = 0
        for _, labels in self.train_prune_loader:
        ...
        for index, item in enumerate(self.model.parameters()):
            if index in self.mask_index:  # if conv filter
                ...
                if len(weight_torch.size()) == 4:  # and filter_pruned_num > 0:
                    ...
                    # print('CS criterion, selected filters | Layer idx - prune filter idx - alpha: {} - {} - {}'.format(prmFiltIdx, filter_index_selected, scaling_factor))
